...
constructor( ) ERC721("StarNotary", "STR")  {
    ...
    function buyStar(uint256 _tokenId) public  payable {
        ...
        if(msg.value > starCost) {
            address payable transferAddress = _make_payable(msg.sender);
           transferAddress.transfer(msg.value - starCost);
        }
    }
    ...
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        ...
        require(msg.sender == address1 || msg.sender == address2, 'Sender does not own any of the tokens');

        //4. Use _transferFrom function to exchange the tokens.
        transferFrom(address1, address2, _tokenId1);
        transferFrom(address2, address1, _tokenId2);
    }

    // Implement Task 1 Transfer Stars
    function transferStar(address _to, uint256 _tokenId) public returns ( address  newAddress) {
        //1. Check if the sender is the ownerOf(_tokenId)
        address address1 = ownerOf(_tokenId);

        require(msg.sender == address1, 'Sender can only transfer star that he owns');
        transferFrom(address1, _to, _tokenId);
